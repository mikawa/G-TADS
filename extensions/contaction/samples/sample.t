...
gameMain: GameMainDef
    ...
    showIntro()
    {
        //new ContActFuse(me, &testCont, 3, gPlayerChar, FooAction, true);
        //new SenseContActFuse(me, &testCont, 3, FooAction, nil, axe, sight);
        //new ContActDaemon(me, &testCont, 3, gPlayerChar, FooAction, true);
        new SenseContActDaemon(me, &testCont, 3, gPlayerChar, FooAction, 
            true, axe, sight);
...
modify me
{
    testCont()
    {
        "Game Clock Time: <<Schedulable.gameClockTime>>\n";
    }
}
...
DefineContIAction(Foo)
    ...
    execActionInitial()
    {
        "execActionIntial() cnt=<<execActionCnt_>>\b";
    }

    execAction() 
    {
        "execAction() cnt=<<execActionCnt_>> ";
    }

    execActionCompleted()
    {
        "\bexecActionCompleted() cnt=<<execActionCnt_>> ";
    }
    execActionInterrupted(issuingActor, targetActor)
    {
        "\bexecActionInterrupted() cnt=<<execActionCnt_>> ";
    }
;
...
VerbRule(Bar)
    'bar' dobjList
    : BarAction
    verbPhrase = 'bar/baring'
;

DefineContTIAction(BazWith)
    pendingCommandRequeue_  = true
...
VerbRule(BazWith)
    'baz' dobjList 'with' singleIobj
    : BazWithAction
    verbPhrase = 'baz/bazing (what) with (what)'
;
...
modify Thing
{
    dobjFor(Bar)
    {
        preCond = [objVisible]
        verify() {}
        action() { "<<gActor.theName>> bar <<self.theName>>. "; }
    }
    iobjFor(BazWith)
    {
        preCond = [objVisible]
        verify() {}
        action() { "<<gActor.theName>> baz <<gDobj.theName>> with <<self.theName>>. "; }
    }
    dobjFor(Frotz)
    {
        preCond = [objVisible]
        ...
        action() 
        {
            "<<gActor.theName>> frotz <<theName>>. ";
            issueContActionInterrupt(gIssuingActor, ContAction, true);
        }
    }
}